...
#ifndef FEPIC_CUTHIL_MCKEE
#define FEPIC_CUTHIL_MCKEE
...
public:
  ...
  struct NodeT {
    ...
    bool operator<(NodeT const& b)  const
    {
    	if (number == b.number)
        return false;
    	else if (pseudo_degree == b.pseudo_degree)
        return number < b.number;
      else
    		return pseudo_degree < b.pseudo_degree;
    }    
    ...
    while (counter < n_nodes)
    {
      ...
      for (; lvl_it!= lvl_it_end;  ++lvl_it) // for each node of this level
      {
        ...
        for (; neib_it != neib_it_end; ++neib_it)  // for each neighbor
        {
          ...
          temp_node.pseudo_degree = std::distance(actual_lvl.begin(),lvl_it) - PENALTY*marks[*neib_it].penalized();
