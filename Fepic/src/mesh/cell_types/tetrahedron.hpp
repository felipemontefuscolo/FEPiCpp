...
public:
  ...
  template<class Derived, class... LabeableArgs>
  Tetrahedron(Eigen::EigenBase<Derived> const& nodes, int order, LabeableArgs... args) :
  ...
  template<class Derived, class... LabeableArgs>
  Tetrahedron(Eigen::EigenBase<Derived> && nodes, int order, LabeableArgs... args) :
                      _Labelable(args...), _nodes(nodes), _order(order)
  {
    FEPIC_CHECK(nodes.size()==numNodes<Simplex<3>>(order), "", std::invalid_argument);
  }

  Tetrahedron() : _nodes(Eigen::Vector4i(-1,-1,-1,-1)), _order(1) {};
  ...
  */
  ...
  static Eigen::Matrix6Xi const& getEdgesLocalNodes(int order)
  {
    FEPIC_CHECK(order<=MAX_CELLS_ORDER, "order not supported yet", std::out_of_range);
    return Tetrahedron::table0[order];
  }

  static Eigen::Matrix4Xi const& getBordersLocalNodes(int order)
  {
    FEPIC_CHECK(order<=MAX_CELLS_ORDER, "order not supported yet", std::out_of_range);
    return Tetrahedron::table1[order];
  }

  static Eigen::MatrixX4i const& getMinimesh(int order)
  {
    FEPIC_CHECK(order<=MAX_CELLS_ORDER, "order not supported yet", std::out_of_range);
    return CellT::table2[order];
  }

  static Eigen::Matrix3Xi const& getOppFLN(int order)
  {
    FEPIC_CHECK(order<=MAX_CELLS_ORDER, "order not supported yet", std::out_of_range);
...
protected:
  ...
  static Eigen::Matrix6Xi _getEdgesLocalNodes(int order)
  {
    // no problem with Tetrahedron::edges_local_vertices
    static const Eigen::Matrix62i edges_local_vertices = ((Eigen::Matrix62i()<<0,1,  1,2,  2,0,  3,0,  3,2,  3,1).finished());

    const int E = order - 1;

    Eigen::Matrix6Xi en(6, order+1);
  ...
  static Eigen::Matrix4Xi _getBordersLocalNodes(int order)
  {
    // no problem with Tetrahedron::borders_local_vertices
    static const Eigen::Matrix43i borders_local_vertices = ((Eigen::Matrix43i()<<1,0,2,  0,1,3,  3,2,0,  2,3,1).finished());

    const int E = order - 1;
    const int C = 4 + E*6;
    const int F = (order-1)*(order-2)/2;

    Eigen::Matrix4Xi fn(4, (order+1)*(order+2)/2);

    // can not use table0, because static initialization dependency =(
    Eigen::Matrix6Xi const& ed_nds = Tetrahedron::_getEdgesLocalNodes(order);
  ...
  static Eigen::MatrixX4i _getMinimesh(int order)
  {
    ...
    */
    Eigen::MatrixX4i              minimesh(order*order*order, 4);
    ...
    auto                          clbegin = coords_list.begin();
    auto                          clend = coords_list.end();
    ...
    for (int i = 0; i < static_cast<int>(coords_list.size()); ++i)
    {
      ...
      if ((clit2 != clend) && (clit3 != clend) && (clit4 != clend))
      {
        ...
        minimesh.row(counter++) = Eigen::Vector4i(i,n2,n3,n4);
      }
  ...
  static Eigen::Matrix3Xi _getOppFLN(int n)
  {
    /* NOT FOR USERS \n
...
#undef PAIR
  ...
  static std::vector<Eigen::Matrix6Xi> _initTable0()
  {
    // the table1 at index n store the local nodes of edges of a cell
    // with order n.
    std::vector<Eigen::Matrix6Xi> table;
  ...
  static std::vector<Eigen::MatrixX4i> _initTable2()
  {
    // the table2 at index n store the minimesh of a cell with order n.
    std::vector<Eigen::MatrixX4i> table;
  ...
  static std::vector<Eigen::Matrix3Xi> _initTable3()
  {
    // the table2 at index n store the minimesh of a cell with order n.
...
public:
  static const Eigen::Matrix43i borders_local_vertices; // faces
  static const Eigen::Matrix62i edges_local_vertices;

  static const std::vector<Eigen::Matrix6Xi> table0; // order / edges local nodes
  static const std::vector<Eigen::Matrix4Xi> table1; // order / borders local nodes
  static const std::vector<Eigen::MatrixX4i> table2; // order / minimeshs
  static const std::vector<Eigen::Matrix3Xi> table3; // order / oppFLN (opposite face local numbers)
...
template<class _Traits>
const Eigen::Matrix43i Tetrahedron<_Traits>::borders_local_vertices = ((Eigen::Matrix43i()<<1,0,2,  0,1,3,  3,2,0,  2,3,1).finished());

template<class _Traits>
const Eigen::Matrix62i Tetrahedron<_Traits>::edges_local_vertices = ((Eigen::Matrix62i()<<0,1,  1,2,  2,0,  3,0,  3,2,  3,1).finished());

template<class _Traits>
const std::vector<Eigen::Matrix6Xi> Tetrahedron<_Traits>::table0 = Tetrahedron::_initTable0();

template<class _Traits>
const std::vector<Eigen::Matrix4Xi> Tetrahedron<_Traits>::table1 = Tetrahedron::_initTable1();

template<class _Traits>
const std::vector<Eigen::MatrixX4i> Tetrahedron<_Traits>::table2 = Tetrahedron::_initTable2();

template<class _Traits>
const std::vector<Eigen::Matrix3Xi> Tetrahedron<_Traits>::table3 = Tetrahedron::_initTable3();
