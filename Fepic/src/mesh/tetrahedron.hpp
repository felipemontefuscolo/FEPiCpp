...
#undef PAIR
  ...
  */ 
  ...
  {
    FEPIC_ASSERT(order<=MAX_CELLS_ORDER, "");
...
protected:
  static matrixi _getEdgesLocalNodes(int order)
  {
    ...
    {
      for (int j = 0; j < 2; ++j)
        en[i][j] = Tetrahedron::edges_local_vertices[i][j];
  ...
  {       
    ...
    matrixi const& ed_nds = Tetrahedron::table0[order];
    
    for (int i = 0; i < 4; i++)
      for (int j = 0; j < 3; j++)
        fn[i][j] = Tetrahedron::borders_local_vertices[i][j];
...
public:  
  ...
  static const std::vector<matrixi> table0; // edges local nodes
  static const std::vector<matrixi> table1; // borders local nodes
  static const std::vector<matrixi> table2; // minimeshs
