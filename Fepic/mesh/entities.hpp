...
static const uint NullIndex = (uint)(-1);

static inline int uintAssert()
{
    static_assert(sizeof(uint)==4, "ERROR: sizeof(unsigned int) must be 4 ...");
    return 0;
}
static bool UINT_SIZE_ERROR = uintAssert();
...
template<class ElmType, class Traits>
class ElementProperties;

template<class Traits> class ElementProperties<Simplex<1>, Traits> {
public:	typedef iEdge<Traits> Type;
};
template<class Traits> class ElementProperties<Simplex<2>, Traits> {
public:	typedef iTriangle<Traits> Type;
};
template<class Traits> class ElementProperties<Simplex<3>, Traits> {
public:	typedef iTetrahedron<Traits> Type;
};
template<class Traits> class ElementProperties<Hypercube<1>, Traits> {
public:	typedef iEdge<Traits> Type;
};
template<class Traits> class ElementProperties<Hypercube<2>, Traits> {
public:	typedef iQuadrangle<Traits> Type;
};
template<class Traits> class ElementProperties<Hypercube<3>, Traits> {
public:	typedef iHexahedron<Traits> Type;
};

template<class Traits> class ElementProperties<iEdge<Traits>, Traits> {
public:	static const int N_borders = 2;
        static const int N_vertices = 2;
};
template<class Traits> class ElementProperties<iTriangle<Traits>, Traits> {
public:	static const int N_borders = 3;
        static const int N_vertices = 3;
};
template<class Traits> class ElementProperties<iTetrahedron<Traits>, Traits> {
...
class UndefElement {
public:
	//UndefElement() {std::cout << "Error: Undefined element" << std::endl;}
...
template<int CellDim, class Cell> // Define o tipo de volume: Cell::dim < 3 ? UndefVol : Cell::Volume
class VolumeDef; 
...
template<int Dim, class Cell> // Define o tipo de face
class FaceDef;
...
/** @class DefaultTraits
 * 
 * Definições padrões que definem estaticamente as características da malha.
 * 
 * O usuário pode fazer ser próprio Traits seguindo este modelo.
 * 
 * Definições obrigatórias:
 * - CellT      := o tipo de célula de maior dimensão
 * - EdgeT
 * - PointT
 * - MeshT
 * - spacedim	:= a dimensão do espaço
 * 
 * 
 */
...
public:
    ...
    DefaultTraits(DefaultTraits const&) = delete; // proteje contra cópia
    ~DefaultTraits() = delete;

	/* as parametrizações em DefaultTraits são opcionais */
...
public:
	 ...
	 */ 
	iMarkedEntity(int Label, bool Isdead=false, bool MustBK=false, bool otherf=false)
	{
 ...
 * 
 * Trata-se da classe iHalfEdge com herança de classe iMarkedEntity.
 */ 
template<class Traits>
class iMarkedHalfEdge : public iHalfEdge<Traits>, public iMarkedEntity
{
public:
	...
	iMarkedHalfEdge(uint cellid, int ith, int label=0) : iHalfEdge<Traits>(cellid, ith),
														 iMarkedEntity(label) {}
	iMarkedHalfEdge() : iHalfEdge<Traits>(), iMarkedEntity() {};
 ...
 * 
 * Trata-se da classe iHalfFace com herança de classe iMarkedEntity.
 */ 
template<class Traits>
class iMarkedHalfFace : public iHalfFace<Traits>, public iMarkedEntity
{
public:
	...
	iMarkedHalfFace() : iHalfFace<Traits>(), iMarkedEntity() {}
 ...
 */ 
template<class Traits>
class iPoint : public iMarkedEntity {
public:
    ...
    typedef Eigen::Matrix<double, Traits::spacedim, 1> VecT;
 	 ...
 	 */ 
	template<class T>
	iPoint(T const& coord, char label=0) : iMarkedEntity(label)
	{
 ...
 */ 
template<class Traits>
class iEdge : public iMarkedEntity {
public:
	 ...
	 */ 
	iEdge(Fepic::vectorui const& nodes, int label=0) : iMarkedEntity(label), _node(nodes)
	{
#ifdef ENTTS_DEBUG
        if (_node.size() < 2)
        {
 ...
 */ 
template<class Traits>
class iPolygon : public iMarkedEntity
{
public:
	 ...
	 */
	iPolygon(Fepic::vectorui const& nodes, int label=0) : iMarkedEntity(label), _node(nodes)
	{
#ifdef ENTTS_DEBUG
...
template<class Traits>
class iPolyhedron : public iMarkedEntity
{
public:
	 ...
	 */ 
	iPolyhedron(Fepic::vectorui const& nodes, int label=0) : iMarkedEntity(label), _node(nodes)
	{
#ifdef ENTTS_DEBUG
        if (_node.size() < static_cast<uint>(N_vertices))
        {
	...
	/** @param nodes vetor com os nós que compõe o polígono.
	 */ 
	iPolyhedron(Fepic::vectorui&& nodes, int label=0) : iMarkedEntity(label), _node(std::move(nodes))
	{
#ifdef ENTTS_DEBUG
...
#define DO_TRANSFORMATION_AND_APPLY for (int i = 0; i != N; ++i)                                             \
                                    {                                                                        \
                                        X[i] = A*Coords[i] + b;                                              \
                                    }                                                                        \
                                                                                                             \
